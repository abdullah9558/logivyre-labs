...
export default async function ProjectPage({params}:{params:Promise<{slug:string}>}){const {slug}=await params;const p=projects[slug as Slug];if(!p)notFound();return <main className="detail-page">
  <nav className="nav shell"><Link className="brand" href="/"><span className="brand-logo"><img className="logo-light" src="/logivyre-logo-light.png" alt="Logivyre Labs"/><img className="logo-dark" src="/logivyre-logo-light.png" alt="" aria-hidden="true"/><img className="logo-dark-v-left" src="/logivyre-logo-light.png" alt="" aria-hidden="true"/><img className="logo-dark-v-right" src="/logivyre-logo-light.png" alt="" aria-hidden="true"/></span></Link><div className="nav-actions"><ThemeToggle/><Link className="nav-cta" href="/">All projects</Link></div></nav>
